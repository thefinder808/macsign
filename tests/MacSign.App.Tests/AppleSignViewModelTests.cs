...
public class AppleSignViewModelTests
{
    ...
    // Stateful fake: tracks which .app basenames have been signed so that, after
    // contents-signing, the SECOND (re-mounted) pre-flight reports them as
    // signed + hardened — letting pre-flight pass and the run proceed to notarize.
    private static FakeRunner StatefulContentsDmg()
    {
        ...
        f.Respond = (file, args) =>
        {
            ...
            if (a.Contains("attach")) { var mp = a[a.IndexOf("-mountpoint") + 1]; Directory.CreateDirectory(Path.Combine(mp, "Demo.app")); return new ProcessResult(0, "", "", false); }
            if (a.Contains("convert") && a.Contains("UDZO")) { File.WriteAllText(a[a.IndexOf("-o") + 1], "x"); return new ProcessResult(0, "", "", false); }
            if (file.EndsWith("codesign", StringComparison.Ordinal) && a.Contains("--sign"))
            {
                if (last.EndsWith(".app", StringComparison.Ordinal)) signed.Add(Path.GetFileName(last));
                return new ProcessResult(0, "", "", false);
            }
            if (a.Contains("--verify"))
                return last.EndsWith(".app", StringComparison.Ordinal) && !signed.Contains(Path.GetFileName(last))
                    ? new ProcessResult(1, "", "not signed", false)
                    : new ProcessResult(0, "", "valid on disk", false); // signed apps + the .dmg itself
            if (a.Contains("-d"))
                return last.EndsWith(".app", StringComparison.Ordinal) && signed.Contains(Path.GetFileName(last))
                    ? new ProcessResult(0, "flags=0x10000(runtime)", "", false)
                    : new ProcessResult(0, "", "", false);
            if (file.EndsWith("xcrun", StringComparison.Ordinal) && a.Contains("notarytool"))
                return new ProcessResult(0, "id: 11111111-1111-1111-1111-111111111111\n  status: Accepted", "", false);
            return new ProcessResult(0, "", "", false); // stapler, spctl, hdiutil convert UDRW/resize/detach
        };
        return f;
    }

    [Fact]
    public async Task SignContentsAndContinue_fixes_contents_then_notarizes_and_staples()
    {
        var dmg = MakeDmg();
        var vm = new AppleSignViewModel(new AppData(), TempStore(), new AppleSigningService(StatefulContentsDmg()))
        { TargetPath = dmg, SelectedIdentity = new SigningIdentity(Sha, IdName), Notarize = true, Staple = true, NotaryProfile = "p" };

        await vm.SignContentsAndContinueCommand.ExecuteAsync(null);

        Assert.True(vm.IsDone);
        Assert.True(vm.ShowOkBanner);        // success — pre-flight passed after the fix, then notarize + staple
        Assert.False(vm.ShowSignContents);
        Assert.False(vm.ShowNotarizeAnyway);
    }

    [Fact]
    public async Task SignContentsAndContinue_failure_shows_error_and_no_recovery()
    {
        var dmg = MakeDmg();
        var f = new FakeRunner();
        f.Respond = (file, args) =>
        {
            ...
            if (file.EndsWith("security", StringComparison.Ordinal)) return new ProcessResult(0, FindIdentityOutput, "", false);
            if (a.Contains("attach")) { var mp = a[a.IndexOf("-mountpoint") + 1]; Directory.CreateDirectory(Path.Combine(mp, "Demo.app")); return new ProcessResult(0, "", "", false); }
            if (a.Contains("--verify")) return new ProcessResult(1, "", "not signed", false);     // inner app needs signing
        ...
        Assert.True(vm.IsDone);
        Assert.True(vm.ShowErrBanner);
        Assert.False(vm.ShowSignContents);
        Assert.False(vm.ShowNotarizeAnyway);
    }

    [Fact]
    public async Task Preflight_block_warns_when_no_entitlements()
    {
        var dmg = MakeDmg();
        var vm = new AppleSignViewModel(new AppData(), TempStore(), new AppleSigningService(UnsignedContentsDmg()))
        { TargetPath = dmg, SelectedIdentity = new SigningIdentity(Sha, IdName), Notarize = true, NotaryProfile = "p", EntitlementsPath = "" };

        await vm.RunCommand.ExecuteAsync(null);

        Assert.True(vm.ShowSignContents);
        Assert.Contains("allow-jit", vm.LogText);   // entitlements footgun warning surfaced
    }
}
